...
for dataset, df in compiled.groupby(['dataset']):
    ...
    smoothed.append(pd.merge(smooth_ratios.reset_index(), smooth_log.reset_index(), on=[
                    'dataset', 'Protein_IDs', 'source', 'type']))
smoothed = pd.concat(smoothed)
smoothed['processing'] = 'smooth'

# Calculate raw values as mean of original (no smoothing)
raw = compiled.groupby(['dataset', 'Protein_IDs', 'source', 'type']).mean()[
    ['ratio', 'log2_ratio']].reset_index()
raw['processing'] = 'raw'
...
thresholded['log_thresholded'] = [1 if ((val > upper_thresh) or (val < lower_thresh)) else np.nan for val,
                                  upper_thresh, lower_thresh in thresholded[['log2_ratio', 'log2_upper_thresh', 'log2_lower_thresh']].values]
thresholded['raw_thresholded'] = [1 if ((val > upper_thresh) or (val < lower_thresh)) else np.nan for val,
                                  upper_thresh, lower_thresh in thresholded[['ratio', 'upper_thresh', 'lower_thresh']].values]

thresholded_raw = thresholded[['dataset', 'Protein_IDs', 'source', 'type', 'ratio', 'processing', 'lower_thresh',
                               'upper_thresh', 'max_thresh', 'raw_thresholded']].copy().rename(columns={'raw_thresholded': 'thresholded'})
